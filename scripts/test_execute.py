...
class TestInvokeCodex:
    def test_invokes_codex_with_correct_args(self, executor, monkeypatch):
        monkeypatch.delenv("CODEX_HARNESS_CMD", raising=False)
    ...
    def test_allows_custom_codex_command(self, executor, monkeypatch):
        monkeypatch.setenv("CODEX_HARNESS_CMD", "custom-codex run --flag")
        ...
        cmd = mock_run.call_args[0][0]
        assert cmd[:3] == ["custom-codex", "run", "--flag"]

    def test_allows_quoted_custom_codex_path(self, monkeypatch):
        monkeypatch.setenv("CODEX_HARNESS_CMD", '"C:\\Program Files\\Codex\\codex.exe" exec --json')
        cmd = ex.StepExecutor._codex_command("PROMPT")

        assert cmd[0] == "C:\\Program Files\\Codex\\codex.exe"
        assert cmd[-1] == "PROMPT"
