...
## Harness 실행
- Harness 작업을 설계하거나 실행할 때는 `$harness` skill을 사용한다.
- phase 계획은 `phases/{task-name}/` 아래의 `index.json`과 `step{N}.md` 파일로 관리한다.
- 자동 실행은 `python scripts/execute.py {task-name}`로 수행한다.
